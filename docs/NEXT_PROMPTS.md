...
### 3. Phase 6 계속 — 6-5 니힐 최종전 연출·문답

**⏸ 트리거**: 없음 (바로 실행 가능). 6-1~6-4 완료됨.

```
Phase 6-5를 진행하자: 니힐 최종전을 연출로 완성한다.
1) 보스 2페이즈 전환(triggerBossPhase2)이 발동될 때 니힐 코어의 페이즈 전환 대사
   (스토리 트리거 'boss:phase2' 등)를 띄운다 — BattleScene에서 useStoryStore.trigger.
2) "문명은 왜 존재해야 하는가" 문답: 최종전 직전(enter:s4는 이미 있음) 또는
   페이즈 전환 시 선택지 2~3개(각자 다른 답) — 어느 답이든 전투로 증명하는 구조,
   선택에 따라 엔딩 화면 문구가 1줄 달라지면 좋음(useStoryStore choices + 엔딩 연동).
3) EndingScreen 확장: conquer:s4 대사(이미 있음) 이후 엔딩 화면에 루멘/니힐 서사
   마무리 문구 반영. 레이븐 영입 여부가 엔딩 한 줄에 반영되면 금상첨화.
완료 기준: 모의/실전 s4 보스전에서 페이즈 전환 대사 + 문답 선택지 동작, 엔딩까지
흐름 확인, npm test 통과.
```

**완료 기준**: MASTER_PLAN 6-5 체크 + 커밋. 이후 6-6(에이스 소품 대화)으로 — Phase 6 마지막 항목.
