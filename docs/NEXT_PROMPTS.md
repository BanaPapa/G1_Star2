...
- **Phase 7-1 완료** (2026-07-07): 내장 전투맵 21종 생성(배경 23장 전부 커버) + 성계 지형(terrains) 기반
  ...
  시작=전술 줌(1.7×)·통상=팬만·격파/크리티컬만 줌 펀치(`combat.camera.*`). 다음은 7-2.
...
### 3. Phase 7 계속 — 7-2 적 구성 확충부터

**⏸ 트리거**: 없음 (바로 실행 가능). 7-1 완료됨 (2026-07-07).

```
Phase 7을 계속하자. docs/MASTER_PLAN.md Phase 7 목록 순서대로:
7-2 적 구성 확충부터 — 성계별 적 조합 차별화, Gravity/Antimatter를 쓰는 적 추가
(무기 25종을 적도 사용). enemies.json 구조와 성계별 enemy 스폰 규칙을 조사 후 진행.
