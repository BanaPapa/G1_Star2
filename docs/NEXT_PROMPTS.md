...
- **남은 12장 (크레딧 1장=1크레딧, 잔액 0.77 — 충전 후 재개)**: shipyard/outpost Lv1 + 5건물 Lv3/Lv5.
  재개 절차: bg_place_daedalus_src를 media_upload → Lv1 프롬프트(완료 3장의 manifest prompt_note·같은 문구 틀) →
  **Lv3/Lv5는 해당 건물 Lv1 생성 job_id를 image_references에 추가**해 증축 연속성 확보(규모·디테일 증가 지시).
  생성 후 원본 저장 → `python scripts/process_buildings.py` 재실행(없는 파일 자동 skip) → manifest 갱신.
