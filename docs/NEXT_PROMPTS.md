...
- **Phase 6(캐릭터·스토리) 전체 완료** (2026-07-07): 루멘 네이밍 → 대사 시스템(StoryDialog) →
  노드별 대사 → 에이스 합류·레이븐 분기 → 니힐 문답·엔딩 분기 → 소품 대화. 다음은 Phase 7.
...
### 3. Phase 7 시작 — 콘텐츠 완성·밸런스

**⏸ 트리거**: 없음 (바로 실행 가능). Phase 6 전체 완료됨 (2026-07-07).

```
Phase 7을 시작하자. docs/MASTER_PLAN.md Phase 7 목록 순서대로:
7-1 전투맵 24종을 성계·전투 카테고리별 배정부터. 현재 맵이 몇 종인지,
배정 구조(mapDefinition/카테고리)가 어떻게 되어 있는지 조사 후 진행하고,
맵이 부족하면 Battle Map Editor 형식으로 신규 맵 데이터를 만들어 채운다.
각 항목 완료마다 MASTER_PLAN 체크 + 커밋 (기존 방식 동일).
```

**완료 기준**: 항목별 MASTER_PLAN 체크 + 커밋. 7-6(완주 플레이테스트)은 사용자와 함께.
