...
### 3. Phase 6 마지막 — 6-6 에이스 소품 대화

**⏸ 트리거**: 없음 (바로 실행 가능). 6-1~6-5 완료됨.

```
Phase 6-6을 진행하자: 에이스 간 상호 대화 소품 — 캐릭터성 강화용 랜덤 1줄.
1) 데이터: story.json에 barks 섹션 신설 — 상황별(전투 승리/입항) 짧은 1줄 대사 풀.
   화자는 영입된 에이스만 후보(카이는 항상). 에이스 2명 이상이면 주고받는 2줄 형식도.
2) 표시: StoryDialog(모달)는 과함 — 가벼운 토스트/말풍선이 적합. 전투 승리 배너나
   입항 화면 한쪽에 "카이: 오늘도 무사히!" 식 1줄. 기존 showFloatingText/배너 재활용 검토.
3) 빈도: 매번은 피로함 — 확률(예: 40%)은 defaultGameConfig에 (하드코딩 금지).
완료 기준: 전투 승리/입항 시 가끔 에이스 1줄 대사 표시, 영입 안 된 에이스는 안 나옴,
npm test 통과. 완료 시 Phase 6 전체 완료 — MASTER_PLAN Phase 6 섹션 마감.
```

**완료 기준**: MASTER_PLAN 6-6 체크 + Phase 6 완료 표기 + 커밋.
