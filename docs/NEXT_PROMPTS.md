...
**단계 B — 건물 스프라이트 15장**: 5종(bld_command_center/research_lab/workshop/shipyard/outpost) ×
Lv1/Lv3/Lv5 변형. 단계 A의 모항 배경(docs/design/generated/bg_place_daedalus_src.png)을 이미지 레퍼런스로
넣어 같은 조명·부감 각도로 생성, 마젠타 크로마키(백색 건물이라 그린 지시 무시됨 — 반물질 사례) +
가공 파이프라인 재활용 → 투명배경 PNG. Lv1→3→5로 규모·디테일 증가(증축 느낌). manifest 갱신 + 커밋.
