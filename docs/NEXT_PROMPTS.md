...
### 3. Phase 7 계속 — 7-3 보스 패턴 강화부터

**⏸ 트리거**: 없음 (바로 실행 가능). 7-1·7-2 완료됨 (2026-07-08).

```
Phase 7을 계속하자. docs/MASTER_PLAN.md Phase 7 목록 순서대로:
7-3 미니보스 가르 / 최종보스 워든 패턴 강화부터 (보스 예외 레이어 활용).
현재 보스 구현(2페이즈·컷인·executeWardenAoe)과 weapons_master_plan.md 보스 예외 규칙을
조사 후 진행. 각 항목 완료마다 MASTER_PLAN 체크 + 커밋 (기존 방식 동일).
```
