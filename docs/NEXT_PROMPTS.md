...
- **Phase 7-3 완료** (2026-07-08): 보스 패턴 강화 — 가르(측후방 DEF -50% 약점 + 전면 주포 +30%), 워든(1페이즈
  ...
  구현 opus-coder 위임 + Codex 리뷰 2건 반영. 다음은 7-4 조합 레시피 확충.
...
### 3. Phase 7 계속 — 7-4 조합 레시피 확충부터

**⏸ 트리거**: 없음 (바로 실행 가능). 7-1·7-2·7-3 완료됨 (2026-07-08).

```
Phase 7을 계속하자. docs/MASTER_PLAN.md Phase 7 목록 순서대로:
7-4 조합 레시피 확충(계열별 1~2개) + 유령시장·떠돌이상인 재고 조정부터.
현재 조합 시스템(레시피 데이터·crafting UI)과 shops.json(유령시장/떠돌이상인) 구조를
