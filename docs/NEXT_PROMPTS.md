...
### 3. Phase 11 시작 — 11-1 세이브 스키마 버전·마이그레이션부터

**⏸ 트리거**: 없음 (바로 실행 가능). Phase 10 완료됨 (2026-07-08).

```
Phase 11(안정화)을 시작하자. docs/MASTER_PLAN.md Phase 11 목록 순서대로:
11-1 세이브 스키마 버전 + 마이그레이션 프레임부터 (출시 후 패치 대비 필수).
현재 세이브 구조(useSaveStore 3슬롯, localStorage 직렬화 대상 스토어들)를 조사 후,
schemaVersion 필드 + 버전별 마이그레이션 체인 + 미지원 버전 안전 처리(로드 거부/백업)를 설계·구현.
