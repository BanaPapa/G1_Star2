...
### 3. Phase 11 계속 — 11-2 번들 분할부터

**⏸ 트리거**: 없음 (바로 실행 가능). 11-1 완료됨 (2026-07-08).

```
Phase 11(안정화)을 계속하자. docs/MASTER_PLAN.md Phase 11 목록 순서대로:
11-2 번들 분할 — Phaser 청크(빌드 경고 1.8MB+)를 dynamic import로 분리하고 로딩 화면 개선.
빌드 출력의 청크 구성(rolldown)과 Phaser import 지점(BattleScreen 등)을 조사 후 진행.
INEFFECTIVE_DYNAMIC_IMPORT 경고(systems.json)도 이 기회에 정리.
