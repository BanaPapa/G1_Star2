...
### 1. Phase 8 — 장소맵 행성·건물 에셋 (HoMM 스타일) — 단계 분할 진행

**⏸ 트리거**: 힉스필드 MCP 세션에서 사용자가 "다음" / "다음 단계"라고 하면 미완료 단계 맨 위부터.

**설계 합의 완료 (2026-07-12 사용자)** — 아래 전 단계의 기준:
- **고정 슬롯**: 모항/행성 배경에 건물 부지 5곳을 구도에 포함. 건물마다 자리 지정(슬롯 좌표는 배경 확정 후 픽셀 좌표로 기록).
  미건설 슬롯은 시안 홀로그램 부지 윤곽으로 표시.
- **레벨 외형 3단계**: 건물당 Lv1/Lv3/Lv5 변형 3장 × 5건물 = 15장. Lv2·4는 직전 변형 재사용 + 'Lv.N' 배지.
- **클릭=패널, 탭 제거**: 맵 위 건물(또는 부지) 클릭 → 해당 시설 패널이 하단 시트로 열림.
  건물이 없는 시설(상점·수리)은 맵 요소(정박장 등)에 매핑. 기존 상단 시설 탭 UI는 제거.
- **컨텍스트 절약**: 한 세션에 한 단계씩. 각 단계 완료 시 해당 단계 삭제 + 커밋.

**단계 A — 배경 4장: ✅ 완료 (2026-07-12)**. bg_space.jpg(로드 에러 해소·코드 무변경) ·
bg_title.png(TitleScreen CSS 적용, 3차 채택 — 1차 상단 하드밴드/2차 함선 IP유사 → 화살촉 자체 디자인) ·
bg_place_daedalus.png(원형 패드 슬롯 **5곳**) · bg_place_planet.png(사각 플랫폼 슬롯 **4곳** — 행성은 건설 건물이
적어 4곳으로 확정). 원본은 docs/design/generated/bg_*_src.png + manifest.

**단계 B — 건물 스프라이트 15장: ⏸ 3/15 진행 (2026-07-13, 힉스필드 크레딧 소진으로 중단)**:
5종(bld_command_center/research_lab/workshop/shipyard/outpost) × Lv1/Lv3/Lv5 변형.
모항 배경(bg_place_daedalus_src.png)을 image_references로 넣어 같은 조명·부감으로 생성,
마젠타 크로마키(백색 건물 — "CRITICAL: every pixel outside the building must be pure magenta (#FF00FF)" 문구 유효 확인).
- **완료 3장**: 사령부/연구소/워크샵 Lv1 → 원본 `bld_*_lv1_src.png` + manifest + `scripts/process_buildings.py`
  (process_hulls 컷아웃 재사용, 회전 없음, 512×512) → `public/assets/bld_*_lv1.png`. 프린지 없음 확인.
- **남은 12장 (크레딧 1장=1크레딧, 잔액 0.77 — 충전 후 재개)**: shipyard/outpost Lv1 + 5건물 Lv3/Lv5.

**⏸ 트리거**: 힉스필드 크레딧 충전 후. 아래 프롬프트를 새 세션에 복붙:

```
G1_Star2 Phase 8 — HoMM 장소맵 단계 B(건물 스프라이트) 재개. 힉스필드 크레딧 충전했어.
현재 3/15 완료(사령부/연구소/워크샵 Lv1 — 커밋 f678486). 남은 12장을 생성해줘:
shipyard/outpost Lv1 + 5건물(command_center/research_lab/workshop/shipyard/outpost) Lv3/Lv5.

절차 (완료 3장과 동일하게):
1) docs/design/generated/bg_place_daedalus_src.png을 media_upload로 올려 image_references로 사용.
2) 모델 nano_banana_2_lite (1k, thinking=HIGH, aspect_ratio 1:1). 프롬프트 틀은 완료 3장의 스타일:
   "Single isolated sci-fi building sprite ... exact same painted concept-art style, high three-quarter
   top-down camera angle and soft upper-left key lighting as the reference space station image" +
   백색 세라믹(#F2F4F8)/레드 트림(#FF3B3B)/시안 발광(#00E0FF) + 건물 홀로 중앙 배치·지형/패드/그림자 금지 +
   "CRITICAL: every pixel outside the building must be pure magenta (#FF00FF)" (백색 건물 필수 문구).
   shipyard Lv1 = 소형 갠트리 드라이독+골조 함선, outpost Lv1 = 요새 벙커+통신탑+포탑 1기.
3) Lv3/Lv5는 같은 건물의 증축: 해당 건물 Lv1 생성 job_id를 image_references에 **배경과 함께 2장** 넣고
   "same building expanded/upgraded" — Lv3 = 부속동·층수 추가, Lv5 = 대형 복합단지·최대 디테일.
   (기존 Lv1 3장의 job_id는 docs/design/generated/manifest.json generation_id 참조)
4) 원본을 docs/design/generated/bld_{건물}_lv{레벨}_src.png로 저장 →
   python scripts/process_buildings.py 실행(없는 파일 자동 skip, 512×512 투명 PNG → public/assets/).
5) 가공 결과 마젠타 프린지 육안 확인 + manifest.json에 12건 추가(category building_homm) +
   이 단계 B 항목을 NEXT_PROMPTS에서 삭제 + npm test·npm run build 통과 확인 후 커밋/푸시.
완료 기준: public/assets/bld_{5건물}_lv{1,3,5}.png 15장 전부 존재. 다음 작업은 단계 C(PlaceScreen).
```

**단계 C — PlaceScreen HoMM 뷰 전환** (opus-coder): 배경 이미지 + 고정 슬롯 5곳(좌표는 단계 A 배경 기준
config `economy.placeMap.slots`) + 건물 스프라이트(레벨→변형 매핑) + 미건설 홀로그램 부지 +
클릭=하단 시트 패널 + 상단 탭 제거 + 상점·수리 맵 요소 매핑. 모의 E2E(입항→건설→클릭) + 테스트 + 커밋.

**단계 D — 성단맵 행성 노드 아트**: 노드 타입별(허브/행성/소행성/성운/보스 등 systems.json 타입 확인)
스프라이트, 현행 이모지(🪐) 대체. StrategyMapScreen 적용 포함.

**단계 E — 전투맵 배경 5장**: 08_backgrounds.md 성계별 5장(극도로 어둡고 균일).
Battle Map Editor 배경 배정과 연결(에디터에서 맵별 배경 확인).

**완료 기준**: 모항 입항 시 배경+건물 실이미지 표시, bg_space 로드 에러 해소, 테스트 통과.
전 단계 완료 시 MASTER_PLAN 8-7(배경)·8-9(행성·건물) 체크.
