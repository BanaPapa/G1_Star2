...
### 3. Phase 6 계속 — 6-4 에이스 합류 연출

**⏸ 트리거**: 없음 (바로 실행 가능). 6-1~6-3 완료됨.

```
Phase 6-4를 진행하자: 에이스 합류를 실제 게임 로직과 연결한다.
1) 현재 세라(s2 정복)·밀라(s3 정복) 합류 "대사"는 6-3에서 넣었지만, 실제
   recruitedAces 부여/함대 사용 가능 처리가 대사와 동기화되는지 확인하고 연결.
   (aces.json joinAt 필드와 useProgressStore.recruitedAces가 어디서 갱신되는지 조사부터)
2) 레이븐 영입 분기: aces.json recruit 조건(s3에서 레이븐 함선을 격파하지 않고
   HP 1 이하 무력화 후 턴 종료) → StoryDialog choices(영입/거절)로 분기 실동작.
   BattleScene에서 조건 감지 → useStoryStore.trigger('raven_offer', onDone) 패턴.
3) 카이는 프롤로그부터 등장(부관 컨셉) — joinAt s1과의 정합은 '처음부터 동행'으로 정리.
완료 기준: s2/s3 정복 후 해당 에이스가 함대 편성에서 사용 가능, 레이븐 분기 양쪽
(영입/미영입) 동작, npm test 통과.
```

**완료 기준**: MASTER_PLAN 6-4 체크 + 커밋. 이후 6-5(니힐 최종전 연출·문답)로.
